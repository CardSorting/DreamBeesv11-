...
/**
 * Wallet Service
 * centralized handling of all user balances (Zaps, Reels) with:
 * - Atomic transactions
...
export class Wallet {
     ...
     * @param {string} requestId - Unique ID for idempotency (required).
     * @param {Object} metadata - Additional info for the ledger (type, description, modelId, etc).
     * @param {string} currency - 'zaps' or 'reels'. Default 'zaps'.
    ...
    static async debit(uid, amount, requestId, metadata = {}, currency = 'zaps', externalTransaction = null) {
        ...
        // But if amount is 0, we can skip balance checks effectively.
        ...
        const run = async (t) => {
            ...
            if (existingTx.exists) {
                ...
                return {
                    ...
                    ...existingTx.data() // Return stored result if we want, or just metadata
                };
            }
            ...
            // Reset "lastActionTime" or similar if needed, but let's keep it generic.
            // Some apps rely on 'lastActionTime' for online status, others for billing.
            // We'll stick to a specific lastTransactionTime for clarity.
