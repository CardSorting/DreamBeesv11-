...
export const handleRateGeneration = async (request) => {
    ...
    try {
        ...
        if (!job.exists || jobData.userId !== uid) {
        ...
        if (jobData.resultImageId) {
            await db.collection('images').doc(jobData.resultImageId).update(update);
        }
...
export const handleModerationVote = async (request) => {
    ...
    try {
        ...
        const result = { consensus: 'pending', userPower: 1, reputationAwarded: 0 };
        await db.runTransaction(async (t) => {
            ...
            if (resultImageId) {
                ...
                if (updates.hidden !== undefined) {
                    imgUpdates.hidden = updates.hidden;
                }
            ...
            if (consensusReached && consensusVerdict) {
                try {
                    ...
                    console.error("Failed to enqueue consensus task", e);
                }
            }
