...
const broadcastWorkerAiChunk = async (userId, requestId, chunk, isFinal = false) => {
 ...
 */
export const handleWorkerAiChat = async (request) => {
...
`;
        ...
        // 1. Run Chat
        const { result, usage } = await WorkerAiService.chatCompletion(enrichedMessages);
        // 2. Deduct Zaps
...
export const handleWorkerAiStream = async (request) => {
...
`;
        ...
        await WorkerAiService.deductUsage(uid, characterId ? 'PERSONA_CHAT' : 'WORKER_AI_CHAT', `wai-stream-${requestId}`, totalTokens);
        // 6. Final chunk
 ...
 */
export const handleCreateWorkerAiTask = async (request) => {
 ...
 */
export const handleCharacterAlchemy = async (request) => {
    const { name, description, isInstant } = request.data;
    if (!request.auth)
        throw new HttpsError('unauthenticated', 'Login required');
    if (!name)
        throw new HttpsError('invalid-argument', 'Name or seed required');
    const uid = request.auth.uid;
    try {
