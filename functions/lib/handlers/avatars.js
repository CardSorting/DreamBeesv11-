...
export const handleGenerateAvatarCollection = async (request) => {
    ...
    if (!theme && !style) {
        throw new HttpsError('invalid-argument', "Theme or Style required");
    }
    try {
        const logRef = requestId ? db.collection('action_logs').doc(requestId) : null;
        const userDisplayName = "DreamBees User";
        return await Billing.runTwoPhase(uid, 'AVATAR_COLLECTION', requestId, { type: 'avatar_forge', theme, style }, async (t) => {
            if (logRef) {
                t.set(logRef, { type: 'avatar_forge', userId: uid, theme, style, createdAt: FieldValue.serverTimestamp() });
            }
        }, async () => {
...
export const handleMintRandomAvatar = async (request) => {
    ...
    try {
        ...
        await Billing.runAtomic(uid, 'AVATAR_MINT', requestId, { type: 'avatar_mint' }, async (t) => {
            ...
            if (logRef) {
                t.set(logRef, { type: 'avatar_mint', userId: uid, avatarId: avatarDoc.id, createdAt: FieldValue.serverTimestamp() });
            }
        });
        return { success: true };
    }
