...
import { checkIpThrottle, checkUserRequestGuards, recordViolation } from "./lib/abuse.js";
...
export const api = onCall({ memory: "512MiB", timeoutSeconds: 300 }, async (request) => {
    ...
    try {
        ...
        if (uid) {
            ...
            preFlightChecks.push(checkUserRequestGuards(uid, action, 1, 10, 0.5, createUserData, clientIp)
                .then((userData) => {
                request.cachedUserData = userData;
            }));
        }
        else {
