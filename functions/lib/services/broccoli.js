...
export class BroccoliFirebase {
    ...
    async materializeTree(treeHash) {
        ...
        if (!treeNode || treeNode.type !== 'tree')
            return {};
        const tree = {};
        ...
        for (const [name, hash] of Object.entries(entries)) {
            ...
            if (node?.type === 'file')
                tree[name] = node.metadata?.content;
            else if (node?.type === 'tree')
                tree[name] = await this.materializeTree(hash);
        }
        BroccoliFirebase.treeCache.set(cacheKey, { tree, timestamp: Date.now() });
        return tree;
    }
    async checkpoint(branch, author) {
...
export class BroccoliPersona {
    ...
    broccoli;
    userId;
    constructor(broccoli, userId) {
        this.broccoli = broccoli;
        this.userId = userId;
    }
    async reconcile(messages, sessionHash) {
        ...
        const identityVector = identityVectorRaw ? JSON.parse(identityVectorRaw).vector : null;
    ...
    Return JSON: { refinedInsights: [...], causalAudit: "Explanation of shift", hypotheses: [...], identityVector: "..." }`;
        ...
        // Final Integration
        let personaData = { insights: [], tasks: [], revision: "Multi-pass reflective synthesis complete." };
        try {
            ...
            if (path.startsWith('persona/knowledge/') && typeof content === 'string') {
                try {
                    ...
                    if (nodeData.vector) {
                        ...
                        if (similarity > 0.7)
                            insights.push({ content: nodeData.content, similarity });
                    }
                }
                catch (e) { }
            }
...
export class BroccoliWorkspace {
    ...
    author;
    staged = {};
    constructor(broccoli, branch, author) {
        this.broccoli = broccoli;
        this.branch = branch;
        this.author = author;
    }
    stage(path, content) { this.staged[path] = content; }
