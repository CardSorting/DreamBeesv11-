...
/**
 * Processes a slideshow generation task.
 */
export const processSlideshowTask = async (req) => {
    ...
    try {
        ...
        for (let i = 0; i < prompts.length; i++) {
            if (results[i]?.status === 'completed' || results[i]?.imageUrl || results[i]?.status === 'failed') {
                continue;
            }
            ...
            try {
                ...
                generatedImageBase64 = result.response.candidates?.[0]?.content?.parts?.[0]?.inlineData?.data || null;
            ...
            if (!generatedImageBase64) {
                ...
                if (resultImageId) {
                    await db.collection("images").doc(resultImageId).update({ slides: results });
                }
                continue;
            }
