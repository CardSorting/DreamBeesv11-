...
import { zapService } from './zap.js';

export const agentService = {
    async tickAgents(derivativeId: string) {
        ...
        if (!derivativeSnap.exists) return;
        const derivative = derivativeSnap.data() as any;
    ...
    async manageCapital(userId: string, delta: number) {
    ...
    async executeSwarmTick(derivativeId: string) {
    ...
    async logThought(userId: string, derivativeId: string, thought: string, context: any) {
    ...
    async saveMacro(userId: string, name: string, sequence: any[]) {
    ...
    async loadMacros(userId: string): Promise<any[]> {
        const snap = await db.collection('agent_macros').where('userId', '==', userId).get();
        return snap.docs.map(doc => doc.data());
    },

    async saveMemory(userId: string, agentId: string, memory: any) {
    ...
    async getMemory(userId: string, agentId: string): Promise<any> {
        const doc = await db.collection('agent_memory').doc(`${userId}_${agentId}`).get();
        return doc.exists ? doc.data()?.memory : null;
    },

    async decideAgentAction(agentId: string, derivativeId: string, agentType: string) {
