...
import { ImageGenerationRequest, sanitizePrompt } from '../domain/models/ImageGenerationRequest.js';
...
export class PromptPreprocessor {
  ...
  static preprocess(
    ...
    const processedPrompt = ImageGenerationPolicy.applyModelQualityTags(
      originalPrompt,
  ...
  static isPromptSafe(prompt: string): boolean {
    // Length validation
    if (prompt.length < 5) return false;
    if (prompt.length > 50000) return false;
