 ...
 */
export const checkInboundSafety = (text: string): SafetyResult => {
 ...
 */
export const checkOutboundSafety = (text: string): SafetyResult => {
    if (!text) { return { safe: true }; }
 ...
 */
export const checkSafetyWithAI = async (text: string, type: 'INBOUND' | 'OUTBOUND' = 'OUTBOUND'): Promise<SafetyResult> => {
    if (!text) { return { safe: true }; }
