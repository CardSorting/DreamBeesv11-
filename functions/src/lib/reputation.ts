...
export class ReputationService {
     ...
     */
    static getTrustLevel(reputation: number) {
        if (reputation >= this.TIERS.GOLD.min) return this.TIERS.GOLD;
        if (reputation >= this.TIERS.SILVER.min) return this.TIERS.SILVER;
     ...
     */
    static calculateVotePower(reputation: number, confidence: number = 1) {
     ...
     */
    static async awardModerationReputation(uid: string, quantity: number) {
     ...
     */
    static async deductReputation(uid: string, quantity: number) {
        const userRef = db.collection('users').doc(uid);
        await userRef.update({
