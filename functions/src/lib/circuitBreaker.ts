...
export enum CircuitState {
  ...
  HALF_OPEN = 'HALF_OPEN' // Temporary state - allows test requests
}
...
export interface CircuitBreakerConfig {
  ...
  resetTimeoutMs: number;       // Time in ms before attempting to close the circuit
  halfOpenMaxAttempts: number;  // Number of successful requests allowed in half-open state
  name: string;                 // Unique identifier for this circuit breaker
  logger?: CircuitBreakerLogger; // Optional custom logger
}

/**
* Logger interface for circuit breaker tracking
*/
...
/**
* Tracks the state progress through circuit states
*/
export interface CircuitBreakerState {
  state: CircuitState;
  failureCount: number;
  lastFailureTime?: Date;
  successCount: number;       // In half-open, tracks successful attempts
...
/**
* Results from circuit breaker execution
*/
export interface CircuitBreakerResult<T> {
  ...
  wasRejected?: boolean;      // Additional info if request was rejected
}

/**
* Default logger implementation
*/
const defaultLogger: CircuitBreakerLogger = {
...
export class CircuitBreaker {
  ...
  private localLog: CircuitBreakerState; // Local memoized state
   ...
   */
  private handleError(error: Error): CircuitBreakerResult<never> {
    this.recordFailure();
    
    const result: CircuitBreakerResult<never> = {
      success: false,
      error,
      circuitState: this.state.state,
      wasRejected: false
   ...
   */
  public reset(immediateIfOpen: boolean = false): void {
