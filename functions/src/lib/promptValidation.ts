...
/**
* Defines the result of a prompt validation check
*/
...
export function validatePrompt(
  ...
  const {
    minLength = SECURITY_THRESHOLDS.MIN_TOKENS,
    maxLength = SECURITY_THRESHOLDS.MAX_TOKENS,
  ...
  const bannedMatch = SECURITY_THRESHOLDS.BANNED_PATTERNS.some(pattern => 
    pattern.test(sanitizedPrompt)
  );
  ...
  if (bannedMatch) {
    ...
    errors.push(
      `Security violation detected: prompt contains suspicious content`,
      `Blocked patterns: ${matchedPatterns.join(', ')}`
    );
  ...
  const aggressiveMatches = aggressiveWords.filter(word => 
    new RegExp(`\\b${word}\\b`, 'gi').test(sanitizedPrompt)
  );
