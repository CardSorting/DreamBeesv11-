...
export class ImageGenerationPolicy {
   ...
   */
  static getResolution(aspectRatio: string): { width: number, height: number } {
   ...
   */
  static applyModelQualityTags(prompt: string, modelId: string): string {
   ...
   */
  static getDefaultSteps(modelId: string): number {
    switch (modelId) {
   ...
   */
  static getDefaultCFG(modelId: string): number {
    switch (modelId) {
      case 'chenkin-noob-xl':
        return 4.0;
      case 'flux-klein-9b':
   ...
   */
  static getDefaultScheduler(modelId: string): string {
    switch (modelId) {
      case 'chenkin-noob-xl':
        return 'Euler a';
   ...
   */
  static requiresHiresFix(modelId: string): boolean {
    switch (modelId) {
      case 'wai-illustrious':
      case 'nova-3d-cg-xl':
        return true;
   ...
   */
  static requiresQualityTags(modelId: string): boolean {
   ...
   */
  static supportsImageEditing(modelId: string): boolean {
   ...
   */
  static getModelDefaults(modelId: string): ModelGenerationConfig {
   ...
   */
  static getMaxSteps(modelId: string): number {
    switch (modelId) {
      case 'chenkin-noob-xl':
        return 50;
      case 'nova-3d-cg-xl':
   ...
   */
  static getMaxCFG(modelId: string): number {
    switch (modelId) {
      case 'chenkin-noob-xl':
        return 20.0;
      default:
   ...
   */
  private static normalizeForRegex(text: string): string {
...
export class ResolutionRules {
  static getFluxTiles(aspectRatio: string): number {
