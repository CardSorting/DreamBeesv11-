...
export const handleDecideAgentAction = async (request: RequestWithAuth<{
    style: string,
    context: any,
    tools: any[]
}>) => {
    ...
    try {
        ...
        const responseText = (await result.response).candidates![0].content.parts[0].text as string;
...
export const handleTickAgents = async (request: RequestWithAuth<{ derivativeId: string }>) => {
    const { derivativeId } = request.data;
    if (!derivativeId) throw new HttpsError('invalid-argument', 'Missing derivativeId');
...
export const handleSaveMemory = async (request: RequestWithAuth<any>) => {
...
export const handleGetMemory = async (request: RequestWithAuth<any>) => {
