 ...
 */
export async function handleDistillRequest(request: RequestWithAuth<{ images: string[], basePack?: any }>) {
    ...
    try {
        ...
        try {
            if (!fs.existsSync(path.dirname(packPath))) { fs.mkdirSync(path.dirname(packPath), { recursive: true }); }
