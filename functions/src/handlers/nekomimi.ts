...
import { handleError, logger, fetchWithTimeout, getS3Client } from "../lib/utils.js";
 ...
 */
export const handleNekomimiTransform = async (request: RequestWithAuth<{ imageBase64: string, mimeType: string, prompt: string }>) => {
    const uid = request.auth?.uid;
    if (!uid) { throw new HttpsError('unauthenticated', "User must be authenticated"); }
    const { imageBase64, mimeType, prompt } = request.data;
    if (!imageBase64 || !prompt) { throw new HttpsError('invalid-argument', "Image and prompt are required"); }
