                ...
                <div className="discovery-container">
                    ...
                    <section className="feed-posts-container" style={{ maxWidth: '600px', margin: '0 auto', paddingBottom: '60px' }}>
                        {images.map((imgItem, index) => {
                            // Determine model info if possible (generation_queue usually has modelId)
                            const itemModel = availableModels.find(m => m.id === imgItem.modelId) || { name: 'Unknown Model', image: '/dreambees_icon.png' };

                            // User display name might not be present in `generation_queue` if not explicitly saved on the doc. 
                            // Usually it's just userId. We can fallback to "DreamBees User" if missing.
