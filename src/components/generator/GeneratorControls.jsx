...
export default function GeneratorControls({
    ...
    return (
        <div style={{ padding: '0', background: 'transparent', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            {/* Enhance Button Bar */}
            <div style={{ padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.2)' }}>
                <button
            ...
            <div className="glass-panel" style={{
                ...
                {activeStyleId ? (
                    <div style={{ padding: '30px', textAlign: 'center', color: 'var(--color-text-muted)' }}>
                        ...
                        <h3 style={{ margin: '0 0 8px 0', color: 'white' }}>{STYLE_REGISTRY.find(s => s.id === activeStyleId)?.label} Style Active</h3>
                        <p style={{ margin: '0 0 16px 0', fontSize: '0.9rem', opacity: 0.7 }}>Prompt will be automatically rewritten in this style.</p>
                        <button onClick={() => setActiveStyleId(null)} className="btn-ghost" style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', textDecoration: 'underline' }}>
                                ...
                                {!activeStyleId && (
                                    ...
                                    >
                                        {generating ? <Loader2 className="animate-spin" size={18} /> : (
                                            <> <Sparkles size={18} style={{ fill: 'currentColor' }} /> {activeStyleId ? 'Restyle' : 'Generate'} </>
                                        )}
                                    </button>
                                )}
                            </div>
