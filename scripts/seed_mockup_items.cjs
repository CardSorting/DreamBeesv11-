...
async function seedMockupItems() {
    console.log('Starting seed of Mockup Items...');
    const batch = db.batch();
    const collectionRef = db.collection('mockup_items');

    for (const item of ITEMS) {
        // Create an ID based on label + category to allow re-seeding without dupes
        const id = `${item.category.toLowerCase()}-${item.label.toLowerCase().replace(/\s+/g, '-')}`;
        const docRef = collectionRef.doc(id);

        batch.set(docRef, {
            ...item,
            id: id // Include ID in data if helpful
        }, { merge: true });
    }

    await batch.commit();
    console.log(`Seeded ${ITEMS.length} mockup items.`);
}

seedMockupItems().catch(console.error);
